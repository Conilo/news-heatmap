...
| SLM misclassifies some `detención` articles as `disturbio` when the body mentions fires triggered by a capture | See `docs/design/extract.md` — planned title-only classification stage with SLM as secondary disambiguation |
| `event_type` is excluded from the cluster key, so a genuine follow-up disturbio and the capture that triggered it land in the same cluster | Acceptable trade-off: prevents splitting one real event across multiple clusters due to SLM label variance. Improved upstream classification (see above) reduces the frequency of this issue. The `event_type` of a merged cluster is resolved by majority vote in `_build_event_row`, so the dominant classification (usually the capture) wins. **Planned fix — post-clustering split stage:** after forming clusters, detect ones with a strong bimodal `event_type` split (e.g. N articles → `disturbio`, M articles → `detención`, neither is noise) and break them into separate events. This is more principled than encoding `event_type` in the cluster key and generalises to any event_type pair, not just disturbio |
